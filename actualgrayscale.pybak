...
def grayScale(range):
  ...
  for px in getPixels(pic): 
    myColor = getColor(px)
    if(distance(myColor, color) > range):    #This is how the color is reserved in an image (using an if statement)
...
def grayScale2(range): 
  ...
  for px in getPixels(pic): 
    myColor = getColor(px)
    if(distance(myColor, color1) > range and distance(myColor, color2) > range):   #By incorporating an "if, and" statement, this will allow the user to reserve the 2 colors they pick
  ...
  repaint(pic)
  file = pickAFile()
  writePictureTo(picture, "/Users/brianalynncampbell/Desktop/edited.jpg")
...
#This program will allow the user to save any manupulations to their desktop so that the user can refer back to the actual image and not just the file. 
